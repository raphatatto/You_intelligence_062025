...
# ─── FUNÇÃO PRINCIPAL ─────────────────────────────────────────
def importar_ucbt(gdb_folder: str | Path, distribuidora: str, ano: int):
    camada = "UCBT_tab"
    ...
    for _, row in df.iterrows():
        try:
            ...
            # Energia
            ene = [int(row.get(f"ENE_{str(m).zfill(2)}") or 0) for m in range(1, 12 + 1)]
            cur.execute("""
                INSERT INTO lead_energia (id, lead_id, ene, potencia)
                VALUES (%s, %s, %s, %s)
                ON CONFLICT (id) DO NOTHING
            """, (id_lead, id_lead, ene, float(row.get("CAR_INST") or 0)))
