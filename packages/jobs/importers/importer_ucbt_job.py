...
# Detecta dinamicamente o nome exato da layer com prefixo “UCBT”
...
def main(
    ...
    try:
        ...
        tqdm.write(f"📖 Lendo camada '{layer_name}' do GDB em {gdb_path}...")
        gdf = gpd.read_file(str(gdb_path), layer=layer_name)
        total = len(gdf)
        tqdm.write(f"   → Encontradas {total} feições.")

        if total == 0:
            tqdm.write("   → Nenhuma feição encontrada; marcando no_new_rows.")
            registrar_status(prefixo, ano, camada, "no_new_rows")
            return

        # 2) DataFrame lead_bruto
        df_bruto = pd.DataFrame({
            "cod_id":            gdf.get("COD_ID"),
        ...
        # 3) DataFrame lead_demanda (média de DIC_01 a DIC_12)
        cols_dic = [f"DIC_{i:02d}" for i in range(1, 13)]
        df_demanda = pd.DataFrame({
            "cod_distribuidora": distribuidora,
            "cod_id":            gdf["COD_ID"],
            "ano":               ano,
            "dem_ponta":         gdf[cols_dic].mean(axis=1),
            "dem_fora_ponta":    gdf[cols_dic].mean(axis=1),
        })

        # 4) DataFrame lead_energia (soma de ENE_01 a ENE_12)
